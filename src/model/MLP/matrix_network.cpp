...
auto MatrixNetwork::FeedForward() -> void {
  for (size_t i = 0; i < m_neurons.size() - 1; i++) {
    ...
    for (size_t j = 0; j < m_neurons[i + 1].row(); j++) {
      m_neurons[i + 1](j, 0) =
...
auto MatrixNetwork::GetLocalGrads(std::vector<double> &LocalGrads,
  ...
  if (LocalGrads.empty()) {
    ...
    for (size_t i = 0; i < m_neurons.back().row(); i++) {
    ...
    for (size_t i = 0; i < m_weights[layer].col(); i++) {
      double teta = 0.;

      for (size_t j = 0; j < m_weights[layer].row(); j++) {
...
void MatrixNetwork::SaveWeights(std::string filename) {
  ...
  for (size_t layer = 0; layer < m_weights.size(); layer++) {
    for (size_t i = 0; i < m_weights[layer].row(); i++) {
      for (size_t j = 0; j < m_weights[layer].col(); j++) {
...
auto MatrixNetwork::GetResult() -> size_t {
  ...
  for (int i = 0; i < m_neurons.back().row(); i++) {
